...
| ADM-10 | Upload an offer banner image | In the offer form, use the image upload | Image uploads and displays in the offers list | Offer form uses the same Cloudinary file upload (`folder=offers`). Sign + Cloudinary upload path verified. Existing Onam banner already displays from Cloudinary. | Pass |
...
| ADM-19 | Site pages editor | `/admin/pages` → Edit About / Terms / Contact → save | Opening Edit calls `GET /api/admin/pages/:key` and populates title + Tiptap **before the editor renders** (Tiptap `content` at init, or `setContent` if the fetch finishes later). Save calls `PATCH /api/admin/pages/:key`. Public pages fetch `GET /api/pages/about`, `GET /api/pages/terms` (**not** `terms-and-conditions`), and `GET /api/pages/contact`. After save, those routes revalidate immediately so the public page shows the new copy without waiting on ISR. | — | |
...
| ADM-22 | Duplicate product name detection | `/admin/products/new` — type a name that already exists in the selected subcategory (wait ~300ms). Repeat on edit after changing name or category. After Confirm, type further so the check runs again. | Dialog: *A product named '[baseName]' already exists in [subcategory]. Suggested name: '[suggestedName]'.* The request sends the name field's **exact current value** (frontend does not strip codes). **Confirm replaces** the entire name + slug with the suggestion — never appends (regression: `"Dining Chair DC-02 DC-2"`). A later check uses the field's new current value, not a cached earlier one. **Keep My Name** closes the dialog, leaves the typed name, and shows an inline uniqueness note. No dialog on edit until name or category actually changes. | — | |
| ADM-23 | Signed image upload converts to WebP and shows size savings | `/admin/products/new` — upload an image (JPEG/PNG/HEIC) | Signature request uses folder `products` (or `categories` / `offers` / `blog` / `reviews`). Cloudinary upload includes every signed field, including `format=webp`. Thumbnail shows original → converted size, e.g. `2.4 MB → 340 KB (86% smaller)`. Stored URL ends in `.webp`. | — | |

---

## Summary (fill in after testing)

- Total tests run: `48` (plus 8 new cases HOME-08, HOME-09, PDP-08, ADM-19, ADM-20, ADM-21, ADM-22, ADM-23 added for reviews / Google widget / pages editor / duplicate-name detection / WebP upload — not yet executed)
- Passed: `39`
