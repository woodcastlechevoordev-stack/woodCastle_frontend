...
| ADM-19 | Site pages editor | `/admin/pages` → Edit About / Terms / Contact → save | Opening Edit calls `GET /api/admin/pages/:key` and populates title + Tiptap **before the editor renders** (Tiptap `content` at init, or `setContent` if the fetch finishes later). Save calls `PATCH /api/admin/pages/:key`. Public `/about`, `/terms-and-conditions`, `/contact` show the updated content. | — | |
