...
| `/about` | About Us — fetches `GET /api/pages/about` (key `"about"`, matches the route this time) — 44-year brand history, Thrissur Chevoor origin story, 100% teak wood commitment (see 3.8) | Long-form content block (SEO value) |
| `/contact` | Contact Us — fetches `GET /api/pages/contact` (key `"contact"`) for the editable portion, plus the fixed details in 3.8a (map, phone, email) — real address/map, phone, email, contact form | JSON-LD `LocalBusiness` schema |
...
| `/terms-and-conditions` | Terms & Conditions — **fetches `GET /api/pages/terms`, key is `"terms"`, NOT `"terms-and-conditions"`** — the route slug and the StaticPage `key` are deliberately different strings; using the URL slug as the lookup key will silently fetch nothing and is the most likely reason edited content doesn't appear | Static content page |
...
| `/admin/pages` | Site content editor — list of editable pages (About, Terms & Conditions, Contact), each opening a simple form (title + rich text content using the same Tiptap component) that calls `PATCH /api/admin/pages/:key`. This is the missing screen that makes the backend's long-standing `StaticPage` support actually usable — until now there was no admin UI for it. **The edit form must call `GET /api/admin/pages/:key` on mount and populate both fields before the editor renders** — for Tiptap specifically, this means passing the fetched content into the editor's `content` option at initialization (e.g. in the `useEditor` config), not setting it via state afterward, since Tiptap doesn't automatically re-render its content just because a React state variable changes later; if the content arrives after the editor has already initialized, update it explicitly via `editor.commands.setContent(fetchedContent)` once the fetch resolves |
...
- **Static content pages (About/Terms/Contact) need faster or on-demand revalidation, not the same 3600s used for products** — an hour-long cache delay feels broken to a client who just edited their Terms page and expects to see it immediately. Either use a short `revalidate` (e.g. 60 seconds) on these three routes specifically, or better: call Next.js's on-demand revalidation (`revalidatePath('/terms-and-conditions')` via a small internal API route) right after a successful `PATCH /api/admin/pages/:key` save, so the edit reflects instantly rather than on the next cache window. If terms edits still don't appear after fixing the key mismatch above, stale ISR caching on a long interval is the next most likely cause to check.
...
- **Image upload component (shared across product, category, offer, blog cover, and review photo forms):** a single reusable drag-and-drop component implementing the exact flow in backend spec section 5b — (1) call `POST /api/admin/upload/signature` to get a signed upload token (now including a `format: "webp"` parameter), (2) upload the file directly from the browser to Cloudinary's API using that token **and the same `format` value from the signature response** — every field returned by the signature endpoint gets sent back to Cloudinary as-is, none of them optional, (3) receive back a `secure_url` that already ends in `.webp` — no client-side conversion needed, Cloudinary did it during the upload, (4) save that URL into the form's `images`/`imageUrl`/`bannerImage`/`customerPhoto` field. Build this component once, reuse it everywhere images are needed — don't reimplement per form. Multi-image forms (products) support reordering, with the first image treated as primary; single-image forms (categories, offers, reviews) just need the one upload slot. Show progress during upload and the resulting image as soon as it's available.
- **Before/after file size display:** once an upload finishes, show both the original and converted size next to the image thumbnail, e.g. "2.4 MB → 340 KB (86% smaller)." The **original size** comes from the browser's own `File` object (`file.size`, in bytes) — known instantly, before the upload even starts, no API call needed. The **converted size** comes from the same Cloudinary upload response used to get `secure_url` — that response already includes a `bytes` field for the final stored file, so no second request is needed for this either. Format both as MB (or KB when under 1 MB) rather than showing raw byte counts, and compute the percentage reduction as `(1 - convertedBytes / originalBytes) * 100`, rounded to a whole number. If a batch of multiple images is uploaded at once (the product form's multi-image case), show this per image, not just an aggregate.
...
- **Duplicate product name detection:** on the product create form (and edit, if the name or category changes), once both a name is typed and a subcategory is selected, debounce and call `GET /api/admin/products/check-duplicate-name?name=&categoryId=` (backend spec section 5a3) — always send the name field's exact current value; the backend handles stripping any previous code, the frontend doesn't need its own logic for that. If `isDuplicate: true`, show a confirmation dialog: *"A product named '[baseName]' already exists in [subcategory]. Suggested name: '[suggestedName]'."* with **Confirm** and **Keep My Name** buttons. Tapping **Confirm** **replaces** the entire name field's value with `suggestedName` (and the slug field, if visible/editable, with `suggestedSlug`) — never append or concatenate onto the existing field value, since that's exactly what produced the `"Dining Chair DC-02 DC-2"` bug in testing. After a Confirm, if the admin keeps typing and the debounced check fires again, it must run against the field's new current value, not a cached earlier one. **Keep My Name** just closes the dialog and leaves the admin's typed name untouched — they can still save it, but should see a brief inline note that this may need a unique name to save successfully, since the backend enforces slug uniqueness regardless of what the frontend suggested.
